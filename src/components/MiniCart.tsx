...
function MiniCart({ isOpen }: MiniCartProps) {
  const { closeMiniCart, cartItems, cartQuantity } = useCart();

  return (
    <Offcanvas show={isOpen} onHide={closeMiniCart} placement="end">
      ...
      <Offcanvas.Body>
        <Stack gap={3}>
          {cartItems.map((item) => (
            <MiniCartItems key={item.id} {...item} />
          ))}
